...
### 11. Усложнение (`superscalar`)

* **Требование варианта:** Суперскалярная организация работы процессора (Deferred store, Parallel flush).
* **Особенности реализации:**
  * Согласно техническому заданию, базовое усложнение суперскалярности спроектировано на уровне схем DataPath и готово к пошаговой интеграции на следующем этапе разработки.
...
| **AND** | `0x30` | Все режимы | 2 | Побитовое И: $Dest \leftarrow Dest \ \& \ Src$ |
| **OR** | `0x31` | Все режимы | 2 | Побитовое ИЛИ: $Dest \leftarrow Dest \ | \ Src$ |
...
Схемы DataPath и ControlUnit, отражающие аппаратную структуру процессора, приведены в документации к лабораторной работе.
